 ...
 * ```
 */
export function persistentJSON<T = unknown> (key: string, initial?: T, params?: PersistentJSONParams<T>): T extends undefined ? unknown : Widen<T> {
  return persistent(key, initial as T, {
    ...params,
    decode: v => JSON.parse(v),
    encode: v => JSON.stringify(v),
  }) as T extends undefined ? unknown : Widen<T>
}
